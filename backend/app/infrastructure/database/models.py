...
class QueryReviewORM(Base):
    ...
    # Static analysis
    static_findings: Mapped[list[Any]] = mapped_column(JSONB, nullable=False, default=list)
    static_query_type: Mapped[str | None] = mapped_column(String(20))
    static_table_refs: Mapped[list[Any]] = mapped_column(JSONB, nullable=False, default=list)

    # Execution plan (nullable — skipped if DB unreachable or DML query)
    plan_findings: Mapped[list[Any] | None] = mapped_column(JSONB)
    ...
    ai_findings: Mapped[list[Any]] = mapped_column(JSONB, nullable=False, default=list)
